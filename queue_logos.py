...
WORKFLOW_TEMPLATE = {
 ...
 "443": { "inputs": { "width": 1024, "height": 1024, "batch_size": 1 }, "class_type": "EmptyLatentImage" },
...
LOGO_PROMPTS = [
    "A minimalist professional vector logo for 'ThaiNights', modern neon typography, glowing hot pink and electric blue colors, a small stylized palm tree silhouette, solid black background, high contrast, clean lines, masterpiece, flat design.",
    "Professional logo design, an elegant emblem for 'ThaiNights', a stylized glowing lotus flower made of neon light tubes, hot pink and cyan color palette, solid black background, symmetrical, high-end, vector style.",
    "Graphic design logo, 'ThaiNights' branding, a sleek artistic silhouette of a beautiful woman's profile, flowing lines, integrated with a circular neon spiral, hot pink neon glow, deep black background, sophisticated, luxury branding."
]

def queue_logos():
    print("🚀 Queuing Logo Designs for ThaiNights...")
    for i, prompt in enumerate(LOGO_PROMPTS, 1):
        workflow = json.loads(json.dumps(WORKFLOW_TEMPLATE))
        workflow["45"]["inputs"]["text"] = prompt
        workflow["471"]["inputs"]["filename_prefix"] = f"ThaiNights_Logo_{i:02d}"
        workflow["9"]["inputs"]["filename_prefix"] = f"ThaiNights_Logo_{i:02d}"
        ...
        try:
            ...
            print(f"  [OK] Logo option {i:02d} queued.")
            time.sleep(0.5)
