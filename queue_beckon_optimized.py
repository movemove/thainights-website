...
# T2V Base Workflow (Wan 2.1 Turbo - 10 Second Config - OPTIMIZED RESOLUTION)
# Resolution reduced to 480x864 to avoid OOM
BASE_T2V_10S_OPT = {
  ...
  "80": { "inputs": { "filename_prefix": "video/ThaiNights_Beckon_Opt", "format": "auto", "codec": "auto", "video-preview": "", "video": ["88", 0] }, "class_type": "SaveVideo" },
...
def queue_jobs():
    print("🚀 Re-Launching 10-Second Beckon Series with Optimized Resolution (480x864)...")
    
    # Interrupt old failing jobs
    try:
    ...
    for i in range(1, 11):
        ...
        prompt = f"Professional 4k video, {view}, natural real-time speed. A {girl} is standing at the entrance of a neon-lit bar in Soi Cowboy, wearing {bikini} and {heels}. She is energetically waving and beckoning customers to come inside with a seductive smile and a playful wink. Red neon reflections on the ground, bustling nightlife energy, perfectly smooth matte skin, high texture detail, stable camera, masterpiece, no slow motion."
        
        print(f"Queuing Optimized 10s Job {i:02d}: {bikini}")
        
        workflow = json.loads(json.dumps(BASE_T2V_10S_OPT))
        workflow["89"]["inputs"]["text"] = prompt
        workflow["80"]["inputs"]["filename_prefix"] = f"video/ThaiNights_BeckonOpt10s_{i:02d}"
...
if __name__ == "__main__":
    queue_jobs()
    print("\n--- Optimized 10-second queue is now active! VRAM pressure reduced. ---")
