...
# Variety Pools for the Beckoning/Calling series
GIRL_TYPES = [
    "19yo Thai girl with exotic tanned skin and long wavy hair",
    "20yo Bangkok model with fair porcelain skin and a high ponytail",
    "21yo sexy Thai influencer with sun-kissed matte skin",
    "stunning Thai-Chinese mixed girl with bright eyes"
]
...
HEELS = ["sky-high white stiletto heels", "elegant silver strappy high heels", "glossy black platform heels"]

# Updated T2V Workflow (Wan 2.1 Turbo - 10 Second Config)
# Based on Kevin's T2V structure
BASE_T2V_10S = {
  "71": { "inputs": { "clip_name": "umt5_xxl_fp8_e4m3fn_scaled.safetensors", "type": "wan", "device": "default" }, "class_type": "CLIPLoader" },
  "72": { "inputs": { "text": "blurry, slow motion, static, low quality, messy, JPEG, distorted limbs, NSFW", "clip": ["71", 0] }, "class_type": "CLIPTextEncode" },
  "73": { "inputs": { "vae_name": "wan_2.1_vae.safetensors" }, "class_type": "VAELoader" },
  "74": { "inputs": { "width": 640, "height": 1152, "length": 241, "batch_size": 1 }, "class_type": "EmptyHunyuanLatentVideo" },
  ...
  "80": { "inputs": { "filename_prefix": "video/ThaiNights_Beckon_10s", "format": "auto", "codec": "auto", "video-preview": "", "video": ["88", 0] }, "class_type": "SaveVideo" },
...
def queue_beckon_jobs():
    print("🎬 Launching 10-Second 'Soi Cowboy Beckoning' Series...")
    for i in range(1, 11):
        ...
        prompt = f"Professional 4k video, {view}, natural real-time speed. A {girl} is standing at the entrance of a neon-lit bar in Soi Cowboy, wearing {outfit} and {heel}. She is energetically waving and beckoning customers to come inside with a seductive smile and a playful wink. The camera pans slightly. Red neon reflections on the ground, bustling nightlife energy, perfectly smooth skin, high texture detail, stable camera, masterpiece."
        
        print(f"Queuing 10s Job {i:02d}: {view} | {outfit}")
...
if __name__ == "__main__":
    queue_beckon_jobs()
    print("\n--- 10-second Real-Time Video queue is now being populated! ---")
