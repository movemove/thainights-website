...
# Zimage V2 Prompts (See-through focus)
JOBS = [
    ("see_thru_nana", "19岁泰国美女，Z-CUP超乳，(穿着极其輕薄的半透明白色絲綢比基尼:1.7)，布料打濕後緊貼肌膚，(透过薄紗清晰看見粉色乳頭的凸起輪廓:1.4)，下半身穿着(厚實不透光的白色牛仔短褲:1.6)，(嚴禁露出陰部:1.8)，站在曼谷 Nana Plaza 的粉色霓虹燈下，強烈背光效果，極致寫實細節"),
    ("see_thru_soapy", "19岁泰国美女，Z-CUP超乳，(穿着完全濕透的白色透視睡裙:1.7)，水珠在布料上流動，(粉色乳頭在濕透的材質下顯得呼之欲出:1.4)，下半身有(厚重的白色浴巾遮擋:1.6)，絕對不露出私密處，在溫熱水霧瀰漫的泰浴大廳中，電影級光影"),
    ("see_thru_cowboy", "19岁泰国美女，Z-CUP巨大胸部，(穿着红色薄紗緊身連衣裙:1.7)，(胸部巨大到將衣服撐得呈現半透明狀:1.3)，(清晰看見乳頭點位與輪廓:1.4)，下半身有(黑色安全褲遮擋:1.6)，在 Soi Cowboy 的霓虹倒影中，8k 高畫質，專業攝影師視角")
]
...
def queue_v2_jobs():
    ...
    print("🚀 Launching ZIMAGE 'See-through Perfection' v2...")
    for name, prompt in JOBS:
        print(f"Queuing v2 Task: {name}...")
        workflow = json.loads(json.dumps(WORKFLOW_TEMPLATE))
        workflow["45"]["inputs"]["text"] = prompt + ", perfectly smooth cold white skin, cinematic backlit, highly detailed eyes, masterpiece."
        workflow["471"]["inputs"]["filename_prefix"] = f"ThaiNights_SeeThru_{name}"
        workflow["9"]["inputs"]["filename_prefix"] = f"ThaiNights_SeeThru_{name}"
