...
# Workflows templates from message 7744 (Zimage Master Formula)
WORKFLOW_TEMPLATE = {
...
LOCATIONS = [
    ("currency_exchange", "在曼谷 SuperRich 换汇店门外，手中拿着一叠泰铢纸币，光影柔和"),
    ("ride_hailing", "在深夜曼谷街头，正准备打开一辆 Grab 专车的车门，背景霓虹闪烁"),
    ("nana_plaza", "在曼谷 Nana Plaza 的霓虹灯下，街道人潮拥挤，动感氛围"),
    ("soi_cowboy", "在雨后的 Soi Cowboy 霓虹灯巷弄中，地面有斑斓的反光"),
    ("metro_guide", "在曼谷 BTS 捷运站的高空月台，背景是城市的万家灯火")
]

def queue_variety_pack():
    print("🚀 Queuing 'Ultimate Imagination' Variety Pack...")
    for slug, loc_desc in LOCATIONS:
        print(f"Queuing: {slug}...")
        for i in range(1, 4): # 3 variants per category
            girl = random.choice(GIRL_TYPES)
            outfit = random.choice(OUTFITS)
            pose = random.choice(POSES)
            
            # Masterpiece formula + Imagination
            prompt = f"韩风冷色调网红美白滤镜，{girl}，有着Z-CUP超乳，细腰，{outfit}，{pose}，{loc_desc}。细腻毛孔清晰可见，overexposure，肤如凝脂，大师级摄影，8k极高画质。"
            
            workflow = json.loads(json.dumps(WORKFLOW_TEMPLATE))
            workflow["45"]["inputs"]["text"] = prompt
            workflow["471"]["inputs"]["filename_prefix"] = f"ThaiNights_Variety_{slug}_{i:02d}"
            workflow["9"]["inputs"]["filename_prefix"] = f"ThaiNights_Variety_{slug}_{i:02d}"
            ...
            try:
                ...
                print(f"  [OK] v{i:02d}: {girl.split('，')[0]} + {outfit}")
                time.sleep(0.5)
