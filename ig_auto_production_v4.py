...
# Load the workflows from the provided JSON content
# (I'm using the JSON structures directly to ensure accuracy)
...
WF_T2I = {
 ...
 "45": { "inputs": { "text": "韩风冷色调网红美白滤镜, 19岁泰国美少女，细腻毛孔与真实绒毛清晰可见，韩式网红妆容。 站在曼谷 Soi Cowboy 的霓虹灯下，白色横条三角比基尼泳衣，胸部巨大Z-CUP，细腰，眼神诱惑，背景霓虹闪烁，景深效果，电影感摄影", "clip": [ "446", 1 ] }, "class_type": "CLIPTextEncode" },
 ...
 "471": { "inputs": { "filename_prefix": "IG/ThaiNights_Detail", "images": [ "470", 0 ] }, "class_type": "SaveImage" },
...
WF_I2V = {
 ...
 "108": { "inputs": { "filename_prefix": "video/IG_Wan2.2_i2v", "format": "auto", "codec": "auto", "video-preview": "", "video": ["129:94", 0] }, "class_type": "SaveVideo" },
...
def run_ig_automation():
    print("🚀 Stage 1: Producing 4K Master Photo...")
    # Randomize seeds
    seed_val = random.randint(1, 10**14)
    WF_T2I["454"]["inputs"]["seed"] = seed_val
    WF_T2I["467"]["inputs"]["seed"] = seed_val
    WF_T2I["479"]["inputs"]["seed"] = seed_val
    
    resp = queue_prompt(WF_T2I)
    prompt_id = resp['prompt_id']
    print(f"  - T2I Job Queued: {prompt_id}. Waiting for 4K results...")
    ...
    while not filename:
        ...
        if prompt_id in history:
            ...
            # Node 480 is the final SeedVR2 output
            if "480" in outputs:
                img = outputs["480"]['images'][0]
                fn = img['filename']
                sub = img.get('subfolder', '')
                filename = f"{sub}/{fn}" if sub else fn
                print(f"  ✅ 4K Photo Ready: {filename}")
                break
        time.sleep(10)

    print("🚀 Stage 2: Producing Wan 2.2 Cinematic Video...")
    WF_I2V["97"]["inputs"]["image"] = filename
    ...
    print(f"  - Automation finished. Check ComfyUI output folder for the IG Reel!")
